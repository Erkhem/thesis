...
/*			ArrayList<Job> jobs12 = new ArrayList<Job>();
	...
	public static void findRegret() throws JsonGenerationException, JsonMappingException, IOException{
		//Write to file JSON an array of uncertain jobs			
		ArrayList<JobUncertainty> uncertainJobs = new ArrayList<>();
		/*
		for(int i=1;i<=n; i++){
			uncertainJobs.add(new JobUncertainty(n));
		}
		mapper.enable(SerializationFeature.INDENT_OUTPUT);
	...
	public static void findCriterionFunction() throws JsonParseException, JsonMappingException, IOException{
